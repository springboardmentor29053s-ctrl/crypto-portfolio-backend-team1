...
@Cacheable(value = "portfolioPerformance", key = "#username + '-' + #days")
    public List<PortfolioPerformancePoint> getPortfolioPerformance(String username, int days) {
        ...
        LocalDateTime fromDate = LocalDateTime.now().minusDays(days);
        List<PortfolioSnapshot> snapshots =
                snapshotRepository.findByUserIdOrderByTimestampAsc(user.getId(), fromDate);
...
/*public PortfolioSummaryResponse getPortfolioSummary(String username) {
        ...
        for (Holding holding : holdings) {
            ...
            if (quantity == null || avgCost == null) continue;

            // 🔥 Get latest price from your CryptoMarketService
            double currentPrice = cryptoMarketService
                    .getCurrentPrice(holding.getAssetSymbol());
